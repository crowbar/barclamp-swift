...
execute "populate-dispersion" do
  ...
  action :run
  ignore_failure true
  only_if "swift -V 2.0 --os-tenant-name #{service_tenant} --os-username #{service_user} --os-password '#{service_password}' --os-auth-url #{keystone_auth_url} stat dispersion_objects 2>&1 | grep 'Container.*not found'"
end
